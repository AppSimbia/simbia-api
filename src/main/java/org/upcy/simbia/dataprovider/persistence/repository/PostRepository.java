...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "select p.* from post p " +
            "join industry i on p.idindustry = i.idindustry " +
            "where p.cactive = '1' and p.cstatus = :status and p.idemployee = :employee", nativeQuery = true)
    List<Post> findAllIndustryByEmployee(@Param("employee") Long id, @Param("status") String status);
